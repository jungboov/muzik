...
public class Utility {
	 private static final String root ="/muzik";
	  ...
	  public static String getCodeValue(String code){
		 ...
		 */
		public static boolean compareDay(String cdate) {
			boolean flag = false;
			List<String> list = getDay();
			if (cdate.equals(list.get(0)) || cdate.equals(list.get(1)) || cdate.equals(list.get(2))) {
				flag = true;
			}
		...
		/** 
		  ...
		  * @return 페이징 생성 문자열
		  */ 
		 public static String paging(int totalRecord, int nPage, int recordPerPage,String url,String no,int bbsid, int nowPage, String col, String word){ 
		   int pagePerBlock = 10; // 블럭당 페이지 수 
...
//		     str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		   ...
		   int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
		   if (nowGrp >= 2){ 
		     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&"+no+"="+bbsid+"&nPage="+_nowPage+"'>이전</A></span>"); 
		   } 
		   ...
		   for(int i=startPage; i<=endPage; i++){ 
		     ...
		     if (nPage == i){ 
		       ...
		       str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&"+no+"="+bbsid+"&nPage="+i+"'>"+i+"</A></span>");   
		     } 
		   } 
		    
		   _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
		   if (nowGrp < totalGrp){ 
		     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&"+no+"="+bbsid+"&nPage="+_nowPage+"'>다음</A></span>"); 
		   } 
		   str.append("</DIV>"); 
		...
		public static int brcount(int guestid, BbsReplyDAO rdao) {
			return rdao.rcount(guestid);
		}
	    //notice paging
...
//		    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		...
		//best paging
		public static String paging5(int total, int nowPage, int recordPerPage, String col, String word,String chartType) {
			int pagePerBlock = 10; 														// 블럭당 페이지 수
			int totalPage = (int) (Math.ceil((double) total / recordPerPage)); 			// 전체 페이지
			int totalGrp = (int) (Math.ceil((double) totalPage / pagePerBlock));		// 전체 그룹
			int nowGrp = (int) (Math.ceil((double) nowPage / pagePerBlock)); 			// 현재 그룹
			...
			if (nowGrp >= 2) {
				str.append("<span class='span_box_1'><A href='./list?col=" + col + "&word=" + word + "&nowPage="
						+ _nowPage+ "&chartType=" + chartType + "'>이전</A></span>");
			}
			...
			for (int i = startPage; i <= endPage; i++) {
				...
				if (nowPage == i) {
					...
					str.append("<span class='span_box_1'><A href='./list?col=" + col + "&word=" + word + "&nowPage=" + i+ "&chartType=" + chartType
							+ "'>" + i + "</A></span>");
				}
			}
			...
			if (nowGrp < totalGrp) {
				str.append("<span class='span_box_1'><A href='./list?col=" + col + "&word=" + word + "&nowPage="
						+ _nowPage+ "&chartType=" + chartType + "'>다음</A></span>");
			}
			str.append("</DIV>");
